                ...
                "render": function (data, type, row) {
                    if (row['endDateTime'] == null) {
                        if (row['level'] == viewBagLevel) {
                            return `<button type="button" class="btn btn-outline-info" onclick="askNextLevel('${row['id']}')"  data-placement="bottom" title="Ask Next Level to Help"><i class="fas fa-question"></i></button> <button type="button" class="btn btn-outline-primary" onclick="viewConvertation('${row['id']}')" data-toggle="modal" data-target="#viewConvertationModal"  data-placement="bottom" title="Chatting With Client"><i class="fas fa-comment"></i></button> <button type="button" class="btn btn-outline-danger" onclick="closeTicket('${row['id']}','${viewBagUserId}')"  data-placement="bottom" title="Close Ticket"><i class="fas fa-times"></i></button></i></button>`;
                        } else {
                            return ` <button type="button" class="btn btn-outline-primary" onclick="viewConvertation('${row['id']}')" data-toggle="modal" data-target="#viewConvertationModal"  data-placement="bottom" title="Chatting With Client"><i class="fas fa-comment"></i></button> <button type="button" class="btn btn-outline-danger" onclick="closeTicket('${row['id']}','${viewBagUserId}')"  data-placement="bottom" title="Close Ticket"><i class="fas fa-times"></i></button></i></button>`;
                        }
...
function handleTicket(caseId, employeeId) {
    var obj = new Object();
    obj.CaseId = parseInt(caseId);
    obj.EmployeeId = employeeId;
    Swal.fire({
        title: 'Konfirmasi Penanganan Data',
        text: 'Apakan Anda yakin untuk menangani Case #' + caseId + 'oleh StaffId #' + employeeId + ' ?',
        type: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'https://localhost:44381/api/Tickets/HandleTicket',
                type: "POST",
                headers: {
...
function createConvertation() {
    ...
    if (obj.EmployeeId < 0 || obj.CaseId < 0 || obj.Message == "") {
        Swal.fire({
            title: 'Error!',
